...
const sendRequestMail = async (request, text, user, cb) => {
  //pronomen for mail
  let pro1 =
    user.gender === "male" ? "he" : user.gender === "female" ? "she" : "he/she";
  let pro2 =
    user.gender === "male"
      ? "him"
      : user.gender === "female"
  ...
  //The Message
  let message = `Hey ${request.dancer.name},

the user ${
    user.name
  } at dancevent has responded to your partnerrequest. This request was posted for the event ${
    request.event.title
  }.
